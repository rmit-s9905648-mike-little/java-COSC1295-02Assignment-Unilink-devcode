...
public class table_setup_reply_job 
{
	...
	public void create_reply_job_table()
	{
		final String DB_NAME = "testDB";
		final String FILE_NAME = "src/model/data_setup_reply_job.txt";
		try {
				Connection con = connectionTest.getConnection(DB_NAME); 
				Statement statement = con.createStatement(); 
				statement.execute("DROP TABLE  REPLY_JOB IF EXISTS;");
				System.out.println("REPLY_JOB Table dropped");
				
				String query= 
						"CREATE TABLE REPLY_JOB "
						+ "("
							+ "REPLY_JOB_POST_ID 			VARCHAR(7) 	NOT NULL,"
							+ "REPLY_JOB_RESPONDER_ID 		VARCHAR(7) 	NOT NULL,"
							+ "REPLY_JOB_VALUE 				DOUBLE 		NOT NULL,"
							+ "PRIMARY KEY (REPLY_JOB_POST_ID,REPLY_JOB_RESPONDER_ID )" 
						+ ");";

				int result = statement.executeUpdate(query);
				if (result == 0) 
				{
					System.err.println("REPLY_JOB table created");
				}
	...
	/*			
				//  Get data from file
							...
							while (scanner.hasNextLine()) 
							{
								...
								if(details.length == 3) 
								{
									ps = con.prepareStatement("INSERT INTO REPLY_JOB VALUES (?,?,?);");
									...
									// REPLY_JOB_VALUE  INTEGER NOT NULL,
